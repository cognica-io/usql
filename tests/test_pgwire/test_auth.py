...
class TestTrustAuth:
    def test_trust_immediate_success(self):
        ...
        response, done = asyncio.get_event_loop().run_until_complete(auth.initial())
        assert done is True
        assert response == b""
...
class TestCleartextAuth:
    def test_cleartext_success(self):
        ...
        response, done = loop.run_until_complete(auth.initial())
        assert done is False
        assert len(response) > 0  # AuthenticationCleartextPassword

        response, done = loop.run_until_complete(auth.step(b"secret123\x00"))
        assert done is True

    def test_cleartext_wrong_password(self):
        auth = CleartextAuthenticator("alice", "secret123")
        loop = asyncio.get_event_loop()
        loop.run_until_complete(auth.initial())

        with pytest.raises(InvalidPassword):
            loop.run_until_complete(auth.step(b"wrong\x00"))
    ...
    def test_cleartext_no_password_configured(self):
        ...
        with pytest.raises(InvalidPassword):
            loop.run_until_complete(auth.step(b"anything\x00"))
...
class TestMD5Auth:
    def test_md5_success(self):
        ...
        response, done = loop.run_until_complete(
            auth.step(outer.encode("utf-8") + b"\x00")
        )
        assert done is True
    ...
    def test_md5_wrong_password(self):
        ...
        with pytest.raises(InvalidPassword):
            loop.run_until_complete(auth.step(b"md5wrong\x00"))
...
class TestScramSHA256Auth:
    def test_scram_full_flow(self):
        ...
        response, done = loop.run_until_complete(
            auth.step(client_first_msg.encode("utf-8"))
        )
        assert done is False
        ...
        response, done = loop.run_until_complete(
            auth.step(client_final.encode("utf-8"))
        )
        assert done is True
